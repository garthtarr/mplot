...
```{r artificial.eg, tidy=FALSE, cache=TRUE}
...
```{r af.stepwise, results='asis', tidy=FALSE, cache=TRUE}
...
```{r binom.eg, results='hide', tidy=FALSE, cache=TRUE}
require(mvtnorm)
n=100
set.seed(11)
p = 4
...
```{r bdat.full.plot, results='asis', tidy=FALSE,cache=TRUE}
data(bodyfat)
...
```{r bfat.stepwise.plot, results='asis', tidy=FALSE,cache=TRUE}
...
```{r vis1.eg, results='asis', tidy=FALSE, warning=FALSE,cache=TRUE}
...
```{r vis, results='hide', tidy=FALSE, cache=TRUE}
data(bodyfat)
full.mod = lm(Bodyfat~.,data=subset(bodyfat,select=-Id))
vis.bf = vis(full.mod, n.cores=4, nvmax = 4)
```
...
If you had not already run the adaptive fence routine on this data set, you could also just specify `mplot(lm1, vis = vis1)` and the fence routine will be run when you click on the fence tab.

Personally I prefer to run the `vis()` and `af()` functions and then feeding those saved objects to the `mplot()` function.  Otherwise, it can take quite a while for `mplot()` to run (especially with glm models).