...
To begin consider the following data set inspired by the `pathoeg` data set in the `MPV` package.
...
Also could look at the variable inclusion plots:

```{r vip, results='asis', tidy=FALSE}
vis1 = vis(mf,redundant = FALSE,nbest = 252)
plot(vis1,which="vip")
```
...
plot(vis1,which="lvk",highlight="x8")
```

```{r boot, results='asis', tidy=FALSE}
plot(vis1,which="boot",highlight="x8")
```
