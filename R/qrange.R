...
qrange <- function(
  ...
  kf <- k.range$k.max
  if (model.type == "lm") {
    cand.models <- summary(leaps::regsubsets(
      x = fixed,
      data = data,
      nbest = 1,
      nvmax = kf,
  ...
  if (length(small.names) > 1) {
    small.ff <- paste(
      yname,
      " ~ ",
      paste(small.names[-1], collapse = "+"),
      sep = ""
    )
  } else {
    small.ff <- paste(yname, "~1")
  }
  ...
  if (length(big.names) > 1) {
    big.ff <- paste(
      yname,
      " ~ ",
      paste(big.names[-1], collapse = "+"),
      sep = ""
    )
  } else {
    big.ff <- paste(yname, "~1")
  }
