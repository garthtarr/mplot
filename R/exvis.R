...
#' boot) as well as variable inclusion plots (vip). The
#' function exvis is (much) slower than vis as it does not
#' leverage the power of leaps - on the other hand it does
#' plot the entire model space (because it actually estimates
#' every model).
#' 
...
exvis=function(mf, nvmax, B=100, lambda.max, nbest=5,
  ...
  doParallel::registerDoParallel(cl.vis)
...
#' exv1 = exvis(lm1)
#' \dontrun{
#' plot(exv1,highlight="x1",which="lvk")
#' }

plot.exvis = function(x,highlight,classic=FALSE,html.only=FALSE,
  ...
  if("vip"%in%which){ # variable inclusion plot
    ...
    if(classic){
      ...
      matplot(x = classic.lambda,jitter(as.matrix(classic.vip.df)),type = "l",
              ylab="Bootstrapped probability",xlab = "Penalty")
      leg.nm = names(classic.vip.df)
      leg.nm = gsub("REDUNDANT.VARIABLE","RV",leg.nm)
      legend("topright",leg.nm,bg="transparent",bty="n",
