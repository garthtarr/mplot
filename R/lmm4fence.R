# uses lme4 package for fitting mixed models
lmmfence = function(fixed,random,method,data,cn,B,exhaustive){
  ...
  if(substr(RHS,1,1)=="."){
    ...
    full.fixed.formula = as.formula(paste(LHS,RHS))
  } else full.fixed.formula = fixed
  mf = lme(full.fixed.formula, random=random, data = data) # full model
  null.fixed.formula = as.formula(paste(fixed[[2]],fixed[[1]],"1"))
  m0 = lme(null.fixed.formula, random=random, data = data) # null model
  ...
  if(Qm0<=UB){
    ...
    cat(deparse(null.fixed.formula), "with random component", deparse(random))
    cat("\n")
    flag = TRUE
  }
  ...
  for(i in 2:k.full){
    ...
    for(j in 1:dim(ms)[1]){
      ...
      fixed.formula = paste(names(X)[1]," ~ ",paste(names(mframe)[-1],collapse="+"),sep="")
      fixed.formula = as.formula(fixed.formula)
      em = lme(fixed=fixed.formula, random=random, data=mframe)
      hatQm = Qm(em,method=method)
      if(hatQm<=UB){
        cat(paste("hatQm:", round(hatQm,2),"; Upper bound:", round(UB,2)),"\n")
        cat(paste("hatQm <= UB:",hatQm<=UB,"\n"))
        cat(deparse(fixed.formula), "with random component", deparse(random))
        cat("\n")
        flag = TRUE
