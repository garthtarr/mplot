...
lmfence <- function(
  ...
  UB <- Qmf +
    cstar * sigMM(k.mod = 1, method = method, k.full = kf, adaptive = adaptive)
  if (Qm0 <= UB) {
    if (trace) {
      txt.fn(Qm0, UB, m0)
    }
    ...
    cat("(Not a candidate model) \n")
  }

  if (cstar < 5) {
    # avoids having to add variables to get the full model
    nvmax <- kf
    prev.nvmax <- nvmax
  } else if (nvmax < 5) {
    prev.nvmax <- nvmax
  ...
  while (prev.nvmax <= kf) {
    ...
    for (i in start:nvmax) {
      ...
      UB <- Qmf +
        cstar *
          sigMM(k.mod = i, method = method, k.full = kf, adaptive = adaptive)
      mnames <- colnames(leaps.cands)[which(leaps.cands[lc.k == i, ] == 1)]
      ff <- stats::as.formula(paste(
        yname,
        " ~ ",
    ...
    if (trace) {
      cat(" (No candidate models found: increasing nvmax) \n", cstar)
    }
    nvmax <- nvmax + 5
  }
}
