...
mplot <- function(mf, ...) {
    ...
    dashboardSidebar(
      sidebarMenu(
        ...
        conditionalPanel(
          ...
          selectInput(
            "highlight",
            label = "Highlight models with:",
            choices = if (lvp.res$use.glmulti) {
              base::all.vars(stats::formula(full.model))[-1]
            } else {
              names(stats::coef(full.model))[
                !names(stats::coef(full.model)) == "(Intercept)"
              ]
            }
          ),
          conditionalPanel(
      ...
      box(
        ...
        ("Report issues here: "),
        HTML(paste(
          "<a href='http://github.com/garthtarr/mplot/issues' target='_blank'>"
        )),
        icon("github"),
        HTML(paste("</a>"))
      )
    ...
    dashboardBody(
      tabItems(
        tabItem(
          tabName = "lvp",
          box(
            title = "Model stability plot",
            status = "primary",
            solidHeader = TRUE,
            width = 12,
            conditionalPanel(
              condition = "input.classic=='FALSE'",
              htmlOutput("lvp.gvis")
            ),
            conditionalPanel(
              condition = "input.classic=='TRUE'",
        ...
        tabItem(
          tabName = "bglmnet",
          box(
            title = "Bootstrapped glmnet variable importance plot",
            status = "success",
            solidHeader = TRUE,
            width = 12,
            collapsible = TRUE,
          ...
          box(
            title = "Bootstrapped glmnet model stability plot",
            status = "success",
            solidHeader = TRUE,
            width = 12,
            collapsible = TRUE,
  ...
  server <- function(input, output) {
    ...
    output$screenui <- renderUI({
      conditionalPanel(
        condition = "output.anyscreen=='TRUE'",
        radioButtons(
          inputId = "screen",
          label = "Screen",
          choices = c("Yes" = TRUE, "No" = FALSE),
    ...
    output$nbestui <- renderUI({
      ...
      conditionalPanel(
        condition = "input.lvp=='lvp' & input.boot_lvp=='No'",
        selectInput(
          "nbest",
    ...
    visdat <- reactive({
      validate(
        need(
          !is.null(lvp.res.screened) | !is.null(lvp.res),
    ...
    afdat <- reactive({
      validate(
        need(
          !is.null(af.res.screened) | !is.null(af.res),
    ...
    bglmnetdat <- reactive({
      validate(
        need(
          !is.null(glmnet.res.screened) | !is.null(glmnet.res),
    ...
    output$lvp.classic <- renderPlot({
      ...
      if (input$boot_lvp == "No") {
        ...
        graphics::plot(
          ...
          which = "boot",
          interactive = FALSE,
          max.circle = input$max.circle,
          text = input$text,
          min.prob = input$min.prob,
          srt = input$srt
        )
      }
    })
